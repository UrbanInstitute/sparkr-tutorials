...
**SparkR/R Operations Discussed**: `ggplot` (`ggplot2.SparkR`), `geom_bar`
...
```{r, collapse=TRUE}
ggplot(df, aes(x = cut)) + geom_bar(binwidth = 0.5)
```

#### Bar graph across grouped data:

##### Weighted bar graph:

We can also use `ggplot2.SparkR` to build weighted bar graphs by specifying a `weight` in the ggplot expression. Note that the default `weight` function is to sum the numerical values of the `weight` column, i.e. the plot replaces frequency count with sums of the `weight` DF column across groups. Furthermore, `ggplot2.SparkR` is not currently (Summer 2016) able to calculate and plot the mean `weight` values across groups. In the following example, we plot the total number of carats across the different `"cut"` quality levels:

```{r, collapse=TRUE}
ggplot(df, aes(cut)) + geom_bar(aes(weight = carat)) + ylab("Total no. of carats")
```
...
Specify binwidth and number of bins
...
Specify binwidth or number of bins
...
[INSERT: R code for layer frequency polygons]
