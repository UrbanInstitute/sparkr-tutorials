...
head(arrange(df2, desc(df2$loan_age_avg), asc(df2$count)))
```
...
head(arrange(df2, "loan_age_avg", "count", decreasing = c(TRUE, FALSE)))
```
...
We can also rename a DF column using the `withColumnRenamed` operation as we discussed in the SparkR Basics I tutorial. The following expression returns a DF that is equivalent to `df`, excluding that we have renamed `"servicer_name"` to `"servicer"`.
...
Let's compare the time required to evaluate the following expressions with and without persistence:
...
#### Converting SparkR DataFrame to local R data.frame:
